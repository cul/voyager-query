...
RSpec.describe 'update resource', type: :request do
  ...
  context 'GET' do
    ...
    context '/records/:id/record.marc' do
      let(:identifier_get_url) { "/records/#{identifier}/record.marc" }
    ...
    context '/records/:bib_id/holdings/:holdings_record_id/record.marc' do
      let(:holdings_identifier) { 1742778 }
      let(:identifier_get_url) { "/records/#{identifier}/holdings/#{holdings_identifier}/record.marc" }
